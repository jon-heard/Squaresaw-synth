...
public class SettingsGroup extends UIPanel implements ButtonListener
{
	...
	public SettingsGroup(Song tracks, int x, int y, int width, int height)
	{
		...
		SpriteActor s;
			...
			addActor(s);
		TextActor copyright = new TextActor("Copyright (c) 2011 Pixelgig", 0, 45);
			addActor(copyright);
			copyright.setX(width-copyright.getWidth()-5);
		TextActor version = new TextActor("ver " + SquareSaw.VERSION, 0, 30);
			addActor(version);
			version.setX(width-version.getWidth()-5);
